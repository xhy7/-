...
describe("homeGateway", () => {
  it("returns cloned homepage data for the app shell", async () => {
    ...
    expect(homePageData.shellStatuses[0].value).toBe("Mock First");
  });
  ...
  it("falls back to the default previews for unknown identifiers", async () => {
    ...
    expect(ancestorPreview).toEqual(ancestorDetailPreviews["su-shi"]);
    expect(ancestorPreview).not.toBe(ancestorDetailPreviews["su-shi"]);
